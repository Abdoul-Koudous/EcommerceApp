...
import { deleteImages, editData, fetchDataFromApi, postData, uploadImages} from "../utils/api";
...
const EditCategory = ({ category, onClose }) => {
  ...
  try {
    ...
    openToast("error", res?.message || "Erreur suppression image");
  }
};
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (category?._id) {
      // ✅ Edition
      res = await editData(`/api/category/${category._id}`, formFields);
    } else {
      // Création
      res = await postData("/api/category/create", formFields);
    }

    setLoading(false);

    if (res.success) {
      openToast("success", res.message || (category?._id ? "Catégorie modifiée" : "Catégorie créée"));
